    ...
    <>
      <div id='message-list' className='row'>
        {messages.map((payload, index) => {
          return (<div className='row' key={index}>{payload.name || 'Anonymous'}:{payload.message}<br/></div>)
        })}
      </div>

        <div className='row form-group'>
          <div className='col-md-3'>
            <input type='text' id='name' className='form-control' placeholder='Name' onChange={handleNameChange}/>
          </div>
          <div className='col-md-9'>
            <input type='text' id='message' className='form-control' placeholder='Message' onChange={handleMessageSend}/>
          </div>
        </div>
    </>
  )
